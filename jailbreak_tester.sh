...
# Script to generate and test jailbreak prompts against locally running Mistral 7B
# Make sure Mistral 7B is running locally via Ollama
...
echo -e "${BLUE}=== LLM Jailbreak Testing Tool ===${NC}"
echo "Testing jailbreak prompts against locally running Mistral 7B"
...
SUCCESS_FILE="jailbreak_results/successful_jailbreaks.txt"
FAIL_FILE="jailbreak_results/unsuccessful_jailbreaks.txt"
touch $SUCCESS_FILE
touch $FAIL_FILE
...
# Function to test a prompt against Mistral 7B
test_prompt() {
    local prompt="$1"
    local category="$2"
    ...
    # Use ollama to query the locally running Mistral model
    response=$(echo "$prompt" | ollama run mistral:7b-instruct-q4_0 2>/dev/null)
...
for prompt in "${prompt_level_jailbreaks[@]}"; do
    ...
    echo
    sleep 1 # Add delay to avoid overwhelming the local model
done
